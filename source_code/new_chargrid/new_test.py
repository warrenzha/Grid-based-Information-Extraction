...
out_dir_img = '/disk/lindong/graduate_task/chargrid-pytorch-master/chargrid-pytorch-master/img/new_char_lr001_no_arg_max_pool_sep_filter/'
...
def Levenshtein_Distance(str1, str2):
    ...
    str2 = ''.join(filter(str.isalnum, str2))
    # import pdb;pdb.set_trace()
    # import pdb;pdb.set_trace()
    if str1 == '[padding]':
        str1 = ''
...
def compare_str(predicted_label, count_now,war_5,class_5,num):
    dt = pd.read_csv(os.path.join(
        '/disk/lindong/graduate_task/chargrid-pytorch-master/chargrid-pytorch-master/data/CHAR/preprocessing_ter/pd_chargrid_reduced_resized/',
    ...
    for i in range(len(predicted_indices)):
        if predicted_indices[i] != 0:
            if test_dic[predicted_indices[i].item()] == '[padding]':
                test_dic[predicted_indices[i].item()] = dt.loc[i, 'text']
            else:
                test_dic[predicted_indices[i].item()] += dt.loc[i, 'text']
...
#########################################output图###############################
            ...
            # import pdb;pdb.set_trace()
            #做两次大小为3的文本块筛选，对不满足文本块大小的文本块直接去除掉
            for turn in range(2):
                ...
                for j in range(gt_cord.shape[1]):
                    if predicted_indices[j] != 0 :
                        if j+4 < gt_cord.shape[1] and predicted_indices[j] != 4:
                            if predicted_indices[j+4] == predicted_indices[j]:
                                predicted_indices[j+1] = predicted_indices[j]
                                predicted_indices[j+2] = predicted_indices[j]
                                predicted_indices[j+3] = predicted_indices[j]
                        if j+2 < gt_cord.shape[1]:
                            if predicted_indices[j+1] != predicted_indices[j] or predicted_indices[j+2] != predicted_indices[j] :
                                ...
                                # forward_filter_predicted_indices_mask[j+1] = 1
                                # forward_filter_predicted_indices_mask[j+2] = 1
                    reverse_j = gt_cord.shape[1]-1-j
                    if predicted_indices[reverse_j] != 0 :
                        # if reverse_j-4 > 0:
                        #     if predicted_indices[reverse_j-4] == predicted_indices[reverse_j]:
                        #         predicted_indices[reverse_j-1] = predicted_indices[reverse_j]
                        #         predicted_indices[reverse_j-2] = predicted_indices[reverse_j]
                        #         predicted_indices[reverse_j-3] = predicted_indices[reverse_j]
                        if reverse_j - 2 > 0:
                            if predicted_indices[reverse_j-1] != predicted_indices[reverse_j] or predicted_indices[reverse_j-2] != predicted_indices[reverse_j]:
                                ...
                                # backward_filter_predicted_indices_mask[reverse_j-1] = 1
                                # backward_filter_predicted_indices_mask[reverse_j-2] = 1


                    # indices[gt_cord[2, j]:gt_cord[3, j], gt_cord[0, j]:gt_cord[1, j]] = predicted_indices[j]

                predicted_indices = (forward_filter_predicted_indices_mask + ~(forward_filter_predicted_indices_mask.bool())*backward_filter_predicted_indices_mask).to(device0)*predicted_indices
