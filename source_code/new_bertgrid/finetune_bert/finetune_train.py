...
import finetune_ChargridDataset
...
file_str = 'new_bertgrid_max_pool_sep_new_weight_finetune'
...
bert = BertModel.from_pretrained(
    r'/home/jinac/anaconda3/envs/zn/lib/python3.7/site-packages/pytorch_pretrained_bert/bert-base-uncased').to(device0)
...
if __name__ == '__main__':
    ...
    trainloader, testloader = finetune_ChargridDataset.get_dataset()

    net = new_ChargridNetwork(HW, C, num_classes, num_anchors)

    model_dir = '/disk/lindong/graduate_task/chargrid-pytorch-master/chargrid-pytorch-master/data/model_output/new_bertgrid_max_pool_sep_new_weight_finetune'
...
########################################################################################
    ...
    class_5_num = torch.zeros((5)).to(device0)
    for epoch in range(num_epochs):

        final_loss = 0.0
        num = 0
        # for inputs, label1, label2, label3 in trainloader:
        for label1, input_ids in trainloader:
            num = num + 1
            inputs = input_ids.to(device0).long()
            ...
            for i in range(inputs.shape[0]):
                single_input_id = inputs[i,:int(torch.nonzero(inputs[i,:]>-1,as_tuple=False)[-1]+1)]
                result = bert(single_input_id.view(1,-1), output_all_encoded_layers=True)
                single_layer_result = result[0][11]
            ...
            for i in range(output1.shape[0]):
                ...
                ########################
                # label_sum,label_counts = torch.unique(gt_label,return_counts=True)
                # for count in range(len(label_sum)):
                #     class_5_num[label_sum[count]] += label_counts[count]
                # import pdb;pdb.set_trace()
                #########################
