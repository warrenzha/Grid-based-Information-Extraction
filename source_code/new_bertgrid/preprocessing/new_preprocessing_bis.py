...
dir_np_bertgrid = '/disk/lindong/graduate_task/chargrid-pytorch-master/chargrid-pytorch-master/data/new_Bert/preprocessing/np_bertgrid'
dir_pd_bertgrid = '/disk/lindong/graduate_task/chargrid-pytorch-master/chargrid-pytorch-master/data/new_Bert/preprocessing/pd_bertgrid'
dir_img_reduced = '/disk/lindong/graduate_task/chargrid-pytorch-master/chargrid-pytorch-master/data/new_Bert/preprocessing/img_reduced_raw'

outdir_np_img_reduced = '/disk/lindong/graduate_task/chargrid-pytorch-master/chargrid-pytorch-master/data/new_Bert/preprocessing_bis/np_img_reduced'
outdir_np_chargrid_reduced = '/disk/lindong/graduate_task/chargrid-pytorch-master/chargrid-pytorch-master/data/new_Bert/preprocessing_bis/np_bertgrid_reduced'
outdir_pd_chargrid_reduced = '/disk/lindong/graduate_task/chargrid-pytorch-master/chargrid-pytorch-master/data/new_Bert/preprocessing_bis/pd_bertgrid_reduced'
outdir_img_reduced = '/disk/lindong/graduate_task/chargrid-pytorch-master/chargrid-pytorch-master/data/new_Bert/preprocessing_bis/img_reduced_raw'
...
def get_raw_img_reduced(img, reduce_x, reduce_y, padding_left, padding_right, padding_top, padding_bot):
    img2 = img
    for i in range(0, padding_top):
        img2 = np.insert(img2, 0, 0, axis=0)
    for i in range(0, padding_bot):
    ...
    import pdb;pdb.set_trace()
    img2_reshaped = img2.reshape(img2.shape[0] // reduce_y, -1, img2.shape[1], 3)
    img2 = np.apply_along_axis(lambda x: np.bincount(x).argmax(), axis=1, arr=img2_reshaped)

    img2_reshaped = img2.reshape(img2.shape[0], img2.shape[1] // reduce_x, -1, 3)
    img2 = np.apply_along_axis(lambda x: np.bincount(x).argmax(), axis=2, arr=img2_reshaped)
...
if __name__ == "__main__":
    ...
    for filename in list_filenames:
        ...
        np_bertgrid = np.load(os.path.join(dir_np_bertgrid,filename),allow_pickle=True)
        pd_bertgrid = pd.read_csv(os.path.join(dir_pd_bertgrid,filename).replace('npy','csv'))
        img = np.load(os.path.join(dir_img_reduced,filename))
        if np.shape(np_chargrid) != (0, 0):
            ...
            np_bertgrid_reduced = get_img_reduced(np_bertgrid, reduce_x, reduce_y, padding_left, padding_right, padding_top, padding_bot)

            img_reduced = get_raw_img_reduced(img, reduce_x, reduce_y, padding_left, padding_right, padding_top, padding_bot)

            pd_bertgrid_reduced = reduce_pd_bbox(pd_bertgrid, padding_left, padding_top, reduce_x, reduce_y)



            np.save(os.path.join(outdir_np_chargrid_reduced, filename).replace('npy', 'npy'), np_bertgrid_reduced)
            pd_bertgrid_reduced.to_csv(os.path.join(outdir_pd_chargrid_reduced, filename).replace('npy', 'csv'))

            plt.imshow(img_reduced)
            plt.savefig(os.path.join(outdir_img_reduced, filename).replace("npy", "png"))
            plt.close()

            plt.imshow(np_bertgrid_reduced)
